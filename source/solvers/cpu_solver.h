    ...
    public:
        ...
        void integrate(const float dt) override
        {
            ...
            detail::parallel_blocks(*_context->pool, _state->bodies.size(),
                ...
                {
                    // Named inside the block so each worker's share shows on its own
                    // thread, rather than the whole pass appearing as one span on the
                    // thread that happened to submit it.
