...
namespace nbody
{
    ...
    // Identical to SimdMortonBarnesHutSolver, but uses a depth-first traversal of the octree
    // rather than a breadth-first traversal.
    //
    // Reordering the radix tree into DFS order is an additional parallel sort, but may result
    // in faster tree traversal.
    class SimdMortonDfsBarnesHutSolver final : public CpuSolver
    {
    public:
