...
namespace nbody::detail
{
    ...
    namespace scalar
    {
            ...
            const int32_t i_count = -1)
        {
            ...
            for (int32_t i_radix_local = 0; i_radix_local < i_radix_count; ++i_radix_local)
            {
                ...
                const RadixNode& radix_node = radix_nodes[i_radix];
                const NodeCount& node_count = node_counts[i_radix];
                const int32_t node_offset = node_offsets[i_radix];

                // a radix node's leafs sit after its internals, child0 before child1
                const int32_t i_leaf_0 = 1 + node_offset + node_count.internals;
                int32_t slot = 0;

                // a non-negative child index is a key rather than a node
                if (radix_node.child0_index >= 0)
                    leaf_nodes[radix_node.child0_index] = i_leaf_0 + slot++;
                if (radix_node.child1_index >= 0)
                    leaf_nodes[radix_node.child1_index] = i_leaf_0 + slot++;
            }
        }
        ...
        namespace
        {
            ...
            inline int32_t find_octree_parent(
                const span<const int32_t> radix_parents,
                const span<const NodeCount> node_counts,
                const span<const int32_t> node_offsets,
                const int32_t i_radix)
            {
            ...
            inline int32_t find_first_octree_node(
                const span<const RadixNode> radix_nodes,
                const span<const NodeCount> node_counts,
                const span<const int32_t> node_offsets,
            ...
            inline int32_t find_octree_child(
                const span<const RadixNode> radix_nodes,
                const span<const NodeCount> node_counts,
                const span<const int32_t> node_offsets,
                const span<const int32_t> leaf_nodes,
                const int32_t i_child)
            {
                return (i_child >= 0)
                    ? leaf_nodes[i_child]
                    : find_first_octree_node(radix_nodes, node_counts, node_offsets, leaf_nodes, -i_child);
            }
            ...
            template <typename MortonT>
            inline int32_t find_octree_next(
                const span<const MortonT> keys,
                const span<const RadixNode> radix_nodes,
                const span<const NodeCount> node_counts,
                const span<const int32_t> node_range_ends,
                const span<const int32_t> node_offsets,
                const span<const int32_t> leaf_nodes,
                const int32_t i_key_end)
            {
            ...
            template <typename MortonT>
            inline int32_t find_octree_level(
                const span<const RadixNode> radix_nodes,
                const span<const MortonT> keys,
                const int32_t i_radix)
            {
                const int32_t i_split = std::abs(radix_nodes[i_radix].child0_index);
                return scalar::cpl(keys[i_split], keys[i_split + 1]) / static_cast<int32_t>(MortonT::modulus);
            }
            ...
            template <typename MortonT>
            inline OctreeBounds<MortonT::modulus> find_octree_bounds(const int32_t level, const typename MortonT::Bits key_bits)
            {
                ...
                {
                    const auto group = (key_bits >> (MortonT::width - modulus * l)) & group_mask;
                    for (size_t axis = 0; axis < modulus; ++axis)
                    {
                        index[axis] = (index[axis] << 1) | static_cast<uint32_t>((group >> (modulus - 1 - axis)) & 1);
                    }
                }
                ...
                OctreeBounds<MortonT::modulus> bounds{ .center = {}, .half_extent = 0.5f * cell_size };
                for (size_t axis = 0; axis < modulus; ++axis)
            ...
            const int32_t i_count = -1)
        {
            ...
            octree_nodes[0] = {
                ...
                .child = (node_counts[0].internals > 0)
                    ? 1 + node_offsets[0]
                    : find_octree_child(radix_nodes, node_counts, node_offsets, leaf_nodes, radix_nodes[0].child0_index),
                .is_leaf = false,
            };
            ...
            for (int32_t i_radix_local = 0; i_radix_local < i_radix_count; ++i_radix_local)
            {
                ...
                for (int32_t i_internal = 0; i_internal < node_count.internals; ++i_internal)
                {
                    ...
                    octree_nodes[i_node] = {
                        ...
                        .parent = (i_internal == 0)
                            ? find_octree_parent(radix_parents, node_counts, node_offsets, i_radix)
                            : i_node - 1,
                        ...
                        .child = (i_internal + 1 < node_count.internals)
                            ? i_node + 1
                            : find_octree_child(radix_nodes, node_counts, node_offsets, leaf_nodes, radix_nodes[i_radix].child0_index),
                ...
                for (int32_t i_child = 0; i_child < 2; ++i_child)
                {
                    ...
                    octree_nodes[i_node].next = find_octree_next(keys, radix_nodes, node_counts, node_range_ends, node_offsets, leaf_nodes, i_key);
                    ...
                    octree_bounds[i_node] = find_octree_bounds<MortonT>(i_leaf_level, keys[i_key].bits());
                }
            }
        }

        // build an octree cache and compute the number of nodes required to hold the octree.
        // caller is then responsible to allocate octree node arrays and call build_octree.
        template <typename MortonT>
        size_t build_octree_cache(span<const MortonT> keys, OctreeCache& cache)
        {
            NBODY_PROFILE_ZONE();
            using OctreeBoundsT = OctreeBounds<MortonT::modulus>;
            ...
            // compute and return the number of octree nodes
            cache.num_octree_nodes = 1 + cache.offsets.back() + cache.node_count_totals.back();
            return cache.num_octree_nodes;
        }

        template <typename MortonT>
        void build_octree(span<const MortonT> keys, OctreeCache& cache, const span<OctreeNode>& octree_nodes, const span<OctreeBounds<MortonT::modulus>>& octree_bounds)
        {
        ...
        // Convenience overload for callers that want the cache back afterwards -- the
        // leaf_nodes map in particular, which build_octree_masses needs and which the span
        // overload above leaves in the caller's cache.
        //
        // This is the overload that owns the sizing. The span overload asserts that the
        // output length matches the node count exactly, so the resize here has to be exact
        // rather than grow-only: a vector left over from a larger tree must shrink back, or
        // that assert fires on the second, smaller build.
        template <typename MortonT>
        ...
        {
            build_octree_cache(keys, cache);
            octree_nodes.resize(cache.num_octree_nodes);
            octree_bounds.resize(cache.num_octree_nodes);
            build_octree<MortonT>(keys, cache, span(octree_nodes), span(octree_bounds));
        }
        ...
        {
            OctreeCache cache;
            build_octree<MortonT>(keys, cache, octree_nodes, octree_bounds);
        }
    ...
    {
        template <typename MortonT>
        size_t build_octree_cache(BS::thread_pool& pool, span<const MortonT> keys, OctreeCache& cache)
        {
            NBODY_PROFILE_ZONE();
            using OctreeBoundsT = OctreeBounds<MortonT::modulus>;
            ...
            // return the number of octree nodes to allocate
            cache.num_octree_nodes = 1 + cache.offsets.back() + cache.node_count_totals.back();
            return cache.num_octree_nodes;
        }

        template <typename MortonT>
        void build_octree(BS::thread_pool& pool, span<const MortonT> keys, OctreeCache& cache, const span<OctreeNode>& octree_nodes, const span<OctreeBounds<MortonT::modulus>>& octree_bounds)
        {
            NBODY_PROFILE_ZONE();
            using OctreeBoundsT = OctreeBounds<MortonT::modulus>;
            ...
            parallel_blocks(pool, cache.radix_nodes.size(), [&](const std::ptrdiff_t begin, const std::ptrdiff_t end)
            {
                NBODY_PROFILE_ZONE_NAMED("build octree chunk");
        ...
        {
            OctreeCache cache;
            build_octree_cache(pool, keys, cache);

            if (octree_nodes.size() != cache.num_octree_nodes)
                octree_nodes.resize(cache.num_octree_nodes);
            if (octree_bounds.size() != cache.num_octree_nodes)
                octree_bounds.resize(cache.num_octree_nodes);
